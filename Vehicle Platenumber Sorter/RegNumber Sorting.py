...
# Define Urls
# base_url = 'https://etraffica.herokuapp.com/'
...
# print(df)
# Input_value = 'VBC179DE'


# declare file path
# import from excel file
path = 'C:/Users/akand/Documents/Pycharm/Chithub Pycharm/eTraffika Pycharm/JenoptiksRegNumber.xlsx'
# import the document with pandas
data = pd.read_excel(path, sheet_name='Sheet1')
# print(data)
# filter out regnumber column
filler = data['regNumber']
# print(filler)
...
def writeRegularPlate(items):
    with open('1. JenoptikRegularPlate.csv', 'a', newline='') as wrt:
        writer = csv.writer(wrt)
        writer.writerow([str(items)])

def NonRegularPlate(items):
    with open('2. JenoptikNonRegularPlate.csv', 'a', newline='') as wrt:
        writer = csv.writer(wrt)
        writer.writerow([str(items)])
